...
const init = async (value, option) => {
  ...
  if (fs.existsSync(folderPath)) {
    console.log(
      '当前目录已有名为 imgcook-app 的项目，请 `cd imgcook-app && yarn start` 后访问 http://localhost:3000'
    );
    return;
  }
  cpAsset(folderPath);
...
const cpAsset = folderPath => {
  ...
  try {
    ...
    childProcess.exec(`cp -rf ${assetsDirname} ${folderPath}`, {}, () => {
      ...
      spinner.succeed(
        '执行 `cd imgcook-app && yarn install && yarn start` 后访问 http://localhost:3000 查看您的预览页面'
      );
      spinner.succeed(
        '在 `imgcook-app` 目录中，您可以 `imgcook pull -a 模块id`，查看您的模块预览效果'
      );
    });
  } catch (error) {
